...
const isEditing = computed(() => !!editingVendor.value);
const modalTitle = computed(() => isEditing.value ? 'Edit Vendor' : 'Add Vendor');
...
<template>
    <div>
        ...
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-white">Vendors</h1>
                <p class="mt-1 text-sm text-gray-400">Manage your consignment vendors</p>
            </div>
            <button
                @click="openCreate"
                class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-violet-500/25 transition-all hover:shadow-violet-500/40 hover:brightness-110 active:scale-[0.98]"
            >
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="border-b border-gray-800/60 bg-gray-900/80">
                        ...
                        <th class="px-6 py-4 font-semibold text-gray-300">Email</th>
                ...
                <tbody class="divide-y divide-gray-800/40">
                    ...
                    >
                        <td class="px-6 py-4 text-gray-500">{{ vendor.id }}</td>
                        ...
                        <td class="px-6 py-4 text-gray-400">{{ vendor.email ?? '—' }}</td>
                        <td class="px-6 py-4 text-gray-400">{{ vendor.phone ?? '—' }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <button
                                    @click="openEdit(vendor)"
                                    class="rounded-lg p-2 text-gray-400 transition-colors hover:bg-gray-800 hover:text-violet-400"
                                    title="Edit"
                                >
                                ...
                                <button
                                    @click="destroy(vendor)"
                                    class="rounded-lg p-2 text-gray-400 transition-colors hover:bg-red-950/50 hover:text-red-400"
                                    title="Delete"
                                >
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
        ...
        <Teleport to="body">
            ...
            >
                <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" @click.self="closeModal">
                    ...
                    >
                        <div v-if="showModal" class="w-full max-w-lg rounded-2xl border border-gray-800/60 bg-gray-900 p-6 shadow-2xl">
                            <h2 class="mb-6 text-lg font-semibold text-white">{{ modalTitle }}</h2>
